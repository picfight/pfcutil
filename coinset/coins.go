...
func (cs *CoinSet) TotalValue() (value pfcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue pfcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue pfcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount pfcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue pfcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue pfcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue pfcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue pfcutil.Amount, coins []Coin) (Coins, error) {
...
func (c *SimpleCoin) Value() pfcutil.Amount {
	return pfcutil.Amount(c.txOut().Value)
}
