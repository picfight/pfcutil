...
type WIF struct {
	...
	// netID is the picfightcoin network identifier byte used when
...
func NewWIF(privKey *pfcec.PrivateKey, net *chaincfg.Params, compress bool) (*WIF, error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + pfcec.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + pfcec.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+pfcec.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+pfcec.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+pfcec.PrivKeyBytesLen]
	privKey, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), privKeyBytes)
...
func (w *WIF) String() string {
	...
	// bytes of checksum.
	encodeLen := 1 + pfcec.PrivKeyBytesLen + 4
	if w.CompressPubKey {
		encodeLen++
	}
	...
	a = paddedAppend(pfcec.PrivKeyBytesLen, a, w.PrivKey.D.Bytes())
	if w.CompressPubKey {
...
func (w *WIF) SerializePubKey() []byte {
	pk := (*pfcec.PublicKey)(&w.PrivKey.PublicKey)
	if w.CompressPubKey {
